...
# File formats {background-color="#272e33"}
...
## Common open-source tabular file formats

Plain text file formats that can be opened using a simple text editor:
...
- [Apache Parquet](https://parquet.apache.org/){target="\_blank"} (`.parquet`): columnar binary format, excellent for large datasets.

:::

## Common open-source geographic file formats

Geographic features such as [points, lines or polygons]{.emphasize}:

::: {.incremental}

- ESRI Shapefile (`.shp`) — widely used, but [technically not open]{.emphasize}.

- GeoJSON (`.geojson`) — JSON-based, human-readable, great for web mapping.

- GeoTIFF (`.tif`) — georeferenced raster data (e.g., satellite imagery, SST).

- [GeoParquet](https://geoparquet.org/){target="\_blank"} (`.parquet`) — adds geospatial types to Apache Parquet ([OGC standard]{.emphasize}).
...
[Spatial formats preserve geometry, projection, and metadata]{.emphasize}, enabling:
...
- Spatial joins and data extraction

- Direct visualization and mapping
...
Original data using `EPSG:3413` (NSIDC Sea Ice Polar Stereographic North):

```{r}
#| label: spatial_formats
...
sf_object <- st_sf(
  ...
  geometry = st_sfc(
    st_point(c(-500000, -1000000)),
    st_point(c(0, -500000)),
    st_point(c(500000, -1500000))
  ),
  crs = 3413
)
...
Reproject to WGS84 (`EPSG:4326`) with a single function call:

```{r}
st_transform(sf_object, crs = 4326)
```

# Data project good practices {background-color="#272e33"}
...
- [Use a scripted language (R, Python, Julia, etc.) to perform analysis or make corrections and save that information in a separate file.]{.emphasize}
...
```{=html}
<div style="border: 1px solid #475258; border-radius: 6px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; font-size: 26px; width: 100%; height: 80vh; display: flex; flex-direction: column; box-sizing: border-box; background-color: #2d353b;">
  <div style="background: #374145; padding: 12px 24px; border-bottom: 1px solid #475258; border-radius: 6px 6px 0 0; color: #9da9a0; display: flex; align-items: center; gap: 8px; flex-shrink: 0;">
    📄 <strong>README.md</strong>
  </div>
  <div style="padding: 24px 40px; line-height: 1.6; flex: 1; display: flex; flex-direction: column; justify-content: space-evenly; color: #d3c6aa;">
    <h1 style="font-size: 1.4em; border-bottom: 1px solid #475258; padding-bottom: 6px; margin: 0; color: #a7c080;">Forest Inventory 2024</h1>
    <div>
      <h2 style="font-size: 1.1em; margin: 0 0 4px 0; color: #a7c080;">Project description</h2>
    ...
    <div>
      <h2 style="font-size: 1.1em; margin: 0 0 4px 0; color: #a7c080;">Directory structure</h2>
      <pre style="background: #374145; padding: 12px 18px; border-radius: 6px; font-size: 22px; line-height: 1.6; margin: 0; color: #d3c6aa;">project/
├── data/
...
└── README.md</pre>
    ...
    <div>
      <h2 style="font-size: 1.1em; margin: 0 0 4px 0; color: #a7c080;">How to run</h2>
      <p style="margin: 0;">Run scripts in order: <code style="background:#374145; color: #dbbc7f; padding: 2px 6px; border-radius: 4px;">01_clean.R</code> → <code style="background:#374145; color: #dbbc7f; padding: 2px 6px; border-radius: 4px;">02_analysis.R</code> → <code style="background:#374145; color: #dbbc7f; padding: 2px 6px; border-radius: 4px;">03_figures.R</code></p>
    </div>
    <div>
      <h2 style="font-size: 1.1em; margin: 0 0 4px 0; color: #a7c080;">Contact</h2>
      <p style="margin: 0;">Jane Doe — <a href="mailto:jane.doe@ulaval.ca" style="color: #7fbbb3;">jane.doe@ulaval.ca</a></p>
    </div>
  </div>
</div>
```
...
For example, we want to remove low value of chlorophyll-a from a dataset. Which of the two code snippets below is better?
...
```r
# Chlorophyll-a below 0.1 mg/m³ is below
# sensor detection limit (SeaBird SBE 19+)
min_chla_mg_m3 <- 0.1
ctd <- ctd[ctd$chla >= min_chla_mg_m3, ]
```
...
# File and variable naming {background-color="#272e33"}
...
# Tidy data {background-color="#272e33"}
...
# {background-image="img/screenshots/bad_excel.png" .stretch data-background-size="contain" data-background-position="center" data-background-repeat="no-repeat"}
...
- R, Python, Julia and other programming languages deal well with this.
...
# Backups {background-color="#272e33"}
...
# Publishing your data {background-color="#272e33"}
...
![](https://media.giphy.com/media/j9Y9vsklHWtjgHOtLk/giphy.gif){width=700 fig-alt="GIF showing a person throwing a book over his shoulder."}
...
- [Scholars Portal Dataverse](https://dataverse.scholarsportal.info/){target="\_blank"}
- [Federated Research Data Repository](https://www.frdr-dfdr.ca/repo/?locale=fr){target="\_blank"}
- [National Terrestrial Ecosystem Monitoring System (NTEMS)](https://opendata.nfis.org/mapserver/nfis-change_eng.html){target="\_blank"}
- [Dryad](https://datadryad.org){target="\_blank"}
- [Catalogue de données de l'OGSL](https://ogsl.ca/fr/){target="\_blank"}
- [Zenodo](https://zenodo.org/){target="\_blank"}
- [Figshare](https://figshare.com/){target="\_blank"}
- [The Dataverse Project](https://dataverse.org/){target="\_blank"}

# Take home messages {background-color="#272e33"}
